...
export class StudyGroupService {
  ...
  static async joinStudyGroup(groupID: number): Promise<{ message: string }> {
    try {
      const response = await axiosInstance.post(
        ENDPOINTS.STUDY_GROUPS_JOIN(groupID)
      );
      return response.data;
    } catch (error: any) {
      throw new Error(
        error.response?.data?.error || 'Failed to join study group'
      );
    }
  }
  ...
  static async leaveStudyGroup(groupID: number): Promise<{ message: string }> {
    try {
      const response = await axiosInstance.post(
        ENDPOINTS.STUDY_GROUPS_LEAVE(groupID)
      );
      return response.data;
    } catch (error: any) {
      throw new Error(
        error.response?.data?.error || 'Failed to leave study group'
      );
    }
  }
  ...
  static async createStudyGroup(formData: FormData): Promise<StudyGroup> {
    try {
      ...
      throw new Error(
        error.response?.data?.error || 'Failed to create study group'
      );
    }
  }
  ...
  static async updateStudyGroup(
    ...
    try {
      await Promise.all([
        axiosInstance.patch(ENDPOINTS.STUDY_GROUPS_UPDATE_NAME(groupID), {
          name,
        }),
        axiosInstance.patch(
          ENDPOINTS.STUDY_GROUPS_UPDATE_DESCRIPTION(groupID),
          { description }
        ),
        axiosInstance.patch(ENDPOINTS.STUDY_GROUPS_UPDATE_PRIVATE(groupID), {
          private: private_,
        }),
      ]);
    } catch (error: any) {
      throw new Error(
        error.response?.data?.error || 'Failed to update study group'
      );
    }
  }

  static async updateStudyGroupImage(
    groupID: number,
    image: File
  ): Promise<void> {
    try {
      ...
      await axiosInstance.patch(
        ENDPOINTS.STUDY_GROUPS_UPDATE_IMAGE(groupID),
        formData,
        {
          headers: { 'Content-Type': 'multipart/form-data' },
        }
  ...
  static async deleteStudyGroup(groupID: number): Promise<{ message: string }> {
    try {
      const response = await axiosInstance.delete(
        ENDPOINTS.STUDY_GROUPS_DELETE(groupID)
      );
      return response.data;
    } catch (error: any) {
      throw new Error(
        error.response?.data?.error || 'Failed to delete study group'
      );
    }
  }
}
